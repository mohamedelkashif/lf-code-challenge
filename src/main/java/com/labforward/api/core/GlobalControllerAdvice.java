...
public class GlobalControllerAdvice extends ResponseEntityExceptionHandler implements ResponseBodyAdvice<Object> {
    ...
    public Object entityValidationException(EntityValidationException ex) {
        ...
        ValidationErrorMessage message = new ValidationErrorMessage(ex.getMessage(), new Date());

        for (FieldError fieldError : fieldErrors) {
            String[] info = Objects.requireNonNull(fieldError.getDefaultMessage()).split(OBJECT_ERROR_DELIMITER);
            if (info.length == 2) {
        ...
        for (ObjectError objectError : objectErrors) {
            String[] info = Objects.requireNonNull(objectError.getDefaultMessage()).split(OBJECT_ERROR_DELIMITER);
            if (info.length == 2) {
                message.addError(info[0], info[1]);
            } else {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        ValidationErrorMessage message = new ValidationErrorMessage(MESSAGE_BAD_REQUEST, new Date());
    ...
    private Object handleObjectCreated(Object o, ServletServerHttpResponse serverHttpResponse) {
        ...
        if (o instanceof EntityCreatedResponse) {
            ...
            serverHttpResponse.getHeaders()
                    .set(HttpHeaders.LOCATION, creationResponse.getLocation().toString());
